...
日志文件默认位于 ``~/.forgeagent/logs/forgeagent.log``，可通过环境变量覆盖：
  * FORGEAGENT_LOG_LEVEL — DEBUG / INFO / WARNING / ERROR（默认 DEBUG）
  * FORGEAGENT_LOG_FILE  — 日志文件绝对路径

日志格式包含时间戳、模块名、级别和消息，方便追踪完整的执行流程。

使用方式（各模块）::

    from forgeagent.core.log import get_logger
...
def _default_log_file() -> Path:
    return Path.home() / ".forgeagent" / "logs" / "forgeagent.log"
...
def _setup() -> None:
    ...
    raw_log_file = os.environ.get("FORGEAGENT_LOG_FILE", str(default_log_file)).strip()
    log_file = Path(raw_log_file or str(default_log_file)).expanduser()
    log_level = os.environ.get("FORGEAGENT_LOG_LEVEL", _DEFAULT_LEVEL).strip().upper()
