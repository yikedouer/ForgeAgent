...
class ForgeReplCommandMixin:
    ...
    def _invoke_skill(self, line: str) -> None:
        ...
        if pb is None:
            ...
            if available:
                ...
                self._console.print("[dim]Add skills to .agents/skills/<name>/SKILL.md[/dim]")
            return
    ...
    def do_skills(self, _arg: str) -> None:
        ...
        if not pbs:
            self._console.print("[dim]No skills installed. Add to .agents/skills/<name>/SKILL.md[/dim]")
            return
        for pb in pbs:
    ...
    def do_export(self, arg: str) -> None:
        output = Path(arg.strip() or f"forgeagent-{self.engine.session_id}.md").expanduser()
