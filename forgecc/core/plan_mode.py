...
Plan 文件持久化到 ~/.forgecc/plans/，可在上下文清除后幸存。
"""
...
def generate_plan_file_path(session_id: str) -> Path:
    """返回 ``~/.forgecc/plans/plan-{session_id}.md``，自动创建目录。"""
    safe_session_id = session_file_stem(session_id)
    plans_dir = Path(
        os.environ.get("FORGECC_PLANS_DIR", str(Path.home() / ".forgecc" / "plans"))
    ).expanduser()
...
## Workflow
1. **Explore**: Read code to understand the task. Use read_file, grep_code, search_file.
